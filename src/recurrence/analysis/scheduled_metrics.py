"""Statistical analysis, causal estimands, and bootstrap estimation for Experiment E05.
...
6. Paired McNemar exact tests
7. Token cost scaling and crossover horizons
"""
...
class CausalEstimandSummary:
    """Summary of a causal contrast (Delta = Acc_A - Acc_B) with paired bootstrap CI."""
    contrast_name: str
    condition_a: str
    condition_b: str
...
class ScheduledReplayAnalysisSummary:
    """Comprehensive S06 benchmark analysis summary across horizons and conditions."""
    total_episodes: int
    total_trials: int
...
def compute_mcnemar_test(
    ...
    """Compute paired McNemar test statistic and 2-tailed p-value with continuity correction."""
    ...
    # Approximate p-value from chi2(1) distribution via error function
    p_val = math.erfc(math.sqrt(stat) / math.sqrt(2.0))
    return stat, max(0.0, min(1.0, p_val))
...
def compute_episode_clustered_bootstrap(
    ...
    # Pre-index accuracy by episode and condition
    ...
    for ep in episodes:
        ...
        ep_acc_a[ep] = acc_a if not pd.isna(acc_a) else 0.0
        ep_acc_b[ep] = acc_b if not pd.isna(acc_b) else 0.0
    ...
    return float(point_delta), ci_lower, ci_upper
...
def analyze_scheduled_replay_results(
    ...
    """Perform comprehensive statistical and causal analysis of Experiment E05 trials."""
    ...
    # 1. Condition Accuracy & Token Profiles
    condition_stats: Dict[str, ConditionAccuracyStats] = {}
    ...
    for name, c_a, c_b in contrasts:
        if c_a in conditions and c_b in conditions:
            point_d, ci_l, ci_u = compute_episode_clustered_bootstrap(
            ...
            is_dist = (ci_l > 0.0 or ci_u < 0.0) or (p_val < 0.05)
    ...
    if "incremental_state" in conditions and "replay_transcript" in conditions:
        # Check if replay prompt tokens grow with horizon T while incremental stays O(1)
        h_tokens = []
        for h in horizons:
            df_h = df[df["horizon_ticks"] == h]
        ...
        # If replay exceeds incremental at some horizon, interpolate

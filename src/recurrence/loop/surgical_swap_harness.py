...
1. Raw graft effect Delta_C and absolute directional displacement P_C.
2. Normalized directional projection alpha_C^logit alongside attribution eligibility counts.
3. Unrelated-donor and permuted-donor specificity controls.
4. Dynamic forward propagation (RGLRU_B -> KV_future_B) verifying the temporal transmission mechanism.
"""
...
from recurrence.interventions.surgical_swaps import swap_stores, add_norm_matched_noise
...
class SwapCondition(str, Enum):
    ...
    NOISE_CONTROL_RGLRU = "noise_control_rglru"
...
class SurgicalSwapRecord:
    ...
    cloze_margin: float
    target_choice: str
    raw_graft_effect: float  # Delta_C = m_graft - m_recipient (PRIMARY CAUSAL ESTIMAND)
...
def evaluate_surgical_swaps(
    ...
    # 2. Tokenize unrelated control pair
    if unrelated_pair is None:
        # Default to a distinct canonical pair
        for cp in CANONICAL_STIMULI_PAIRS:
            if cp.pair_id != pair.pair_id:
                unrelated_pair = cp
                break

    if unrelated_pair is not None and tokenizer is not None:
    ...
    # 4. Unroll Branch A, Branch B, Sham A2, and Unrelated Control
    ...
    for current_lag in sorted(target_lags):
        ...
        interventions = [
            ...
            (SwapCondition.NOISE_CONTROL_RGLRU, add_norm_matched_noise(state_a, "rglru", seed=seed), margin_intact_a, margin_intact_a, z_a, z_a, "none"),
        ...
        for cond, grafted_state, rec_m, don_m, z_rec, z_don, donor_label in interventions:
            ...
            # Primary Causal Estimand: Raw Graft Effect Delta_C
            raw_delta = m - rec_m
            ...
            records.append(
                SurgicalSwapRecord(
                    ...
                    raw_graft_effect=round(raw_delta, 4),
...
def evaluate_mediational_propagation(
    ...
    """Causally test if RG-LRU(B) propagates history into subsequently generated KV representations.
    ...
    Measure whether newly generated KV representations in S_graft migrate toward S_B.
    """
    if tokenizer is not None:
        prefix_tokens = tokenizer.encode(pair.prefix, add_special_tokens=False)
        event_a_tokens = tokenizer.encode(pair.event_a, add_special_tokens=False)
    ...
    # 6. Measure Key/Value distances between branches in generated future cache
    d_med_to_a = 0.0
    d_med_to_b = 0.0
    d_a_to_b = 0.0
    ...
    for l in end_state_a.kv:
        ...
        d_med_to_a += float(torch.norm(k_med - k_a) / (torch.norm(k_a) + 1e-8))
        d_med_to_b += float(torch.norm(k_med - k_b) / (torch.norm(k_b) + 1e-8))
        d_a_to_b += float(torch.norm(k_a - k_b) / (torch.norm(k_b) + 1e-8))

    d_med_to_a /= max(n_layers, 1)
    d_med_to_b /= max(n_layers, 1)
    d_a_to_b /= max(n_layers, 1)

    # Migration index: positive means grafted KV migrated toward donor B
    if d_a_to_b > 1e-6:
        kv_migration_index = (d_med_to_a - d_med_to_b) / d_a_to_b
    else:
        kv_migration_index = 0.0
    ...
    return {
        ...
        "d_med_to_a": round(d_med_to_a, 4),
        "d_med_to_b": round(d_med_to_b, 4),
        "d_a_to_b": round(d_a_to_b, 4),
        "kv_migration_index": round(kv_migration_index, 4),
        "propagated_history_to_future_kv": bool(kv_migration_index > 0),
    }

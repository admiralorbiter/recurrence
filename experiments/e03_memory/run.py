...
def generate_e03_markdown_report(
    ...
    lines.extend([
        ...
        f"- **Retained Target Facts (Exact Key-Value Association):** {mean_distortion.retained_target_facts} ({1.0 - mean_distortion.omission_rate - mean_distortion.mutation_rate:.1%})",
        f"- **Exact KV Omission Rate (Target Key String Absent from Summary):** {mean_distortion.omission_rate:.1%} ({mean_distortion.omitted_target_facts}/{mean_distortion.total_target_facts})",
        f"- **Exact Association Mutation Rate (Key Present but Bound Value Altered):** {mean_distortion.mutation_rate:.1%} ({mean_distortion.mutated_facts}/{mean_distortion.total_target_facts})",
        f"- **Unsupported val_* Strings Detected:** {mean_distortion.unsupported_intrusions}",
        ...
        f"- **Memory Fidelity vs. Utility Note:** While exact-string KV reproduction was low (2/18), Model Summary achieved 77.8% on Delayed KV forced choice, indicating that noisy or lossy narrative traces still preserve recognition cues.",
        f"",
        f"---",
        f"",
        f"## 4. Scientific Takeaways for Level 1 & Horizon 1",
        f"",
        f"1. **Observed Static Explicit-Memory Profile:** Explicit external history substantially improves first-order performance over fresh invocation on this synthetic battery.",
        ...
        f"3. **Structured State Trade-off:** Structured State traded static read performance for a smaller, explicitly typed and manipulable representation. It is selected for S05 for semantic coverage and experimental controllability rather than static superiority.",
        f"4. **Autobiographical Narrative Reliability:** Unconstrained narrative consolidation showed high exact-string omission and mutation, though recognition remained viable under forced choice, demonstrating that exact fidelity and recognition utility diverge.",
        f"5. **Combined Condition Observation:** Combined modestly exceeded Structured State on aggregate accuracy (66.7% vs 64.3%), but at 730 prompt tokens it was Pareto-dominated by Full Transcript (81.0% at 499 tokens); the contributions of length, redundancy, ordering, and narrative conflict remain unresolved.",
        f"6. **Transition to Sprint S05:** S04 measured whether a model can read state; Sprint S05 will measure whether an autonomous system can maintain and update that state over time.",
    ])

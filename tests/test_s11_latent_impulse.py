"""Sprint S11: Latent Impulse Response & Store Localization Test Suite (Hardened).

Validates the stimuli generators, dynamic architectural lag grid, residency boundaries,
normalized cross-store distance metrics, layer traces, and non-mutating cloze retrieval probes:
...
3. Event A and Event B stimuli have strictly equal token length across all 20 stimulus pairs.
...
7. Cloze probe uses detached branch clone without advancing main cache position.
...
11. End-to-end execution generates summary records and layer-level traces.
"""
...
def test_matched_branches_receive_identical_filler():
    ...
    pool = [10, 15, 20, 25, 30, 35, 40]
    length = 20
    seed = 42

    for regime in ["constant", "random", "natural", "interfering"]:
        f1 = get_filler_tokens_for_regime(regime, length=length, seed=seed, audited_pool=pool)
        f2 = get_filler_tokens_for_regime(regime, length=length, seed=seed, audited_pool=pool)
...
def test_event_variants_have_equal_token_length():
    """Verify that all 20 canonical stimulus pairs have equal word/token lengths for Event A and Event B."""
    assert len(CANONICAL_STIMULI_PAIRS) == 20, f"Expected 20 canonical stimulus pairs, got {len(CANONICAL_STIMULI_PAIRS)}"
...
def test_impulse_creates_nonzero_initial_separation(test_adapter: RecurrentGemmaAdapter):
    ...
    assert len(layer_records) > 0, "Layer records must be populated at L=0"
...
def test_sham_pair_stays_at_numerical_floor(test_adapter: RecurrentGemmaAdapter):
    ...
    pair = CANONICAL_STIMULI_PAIRS[0]
    records, _ = evaluate_impulse_trajectory(
        adapter=test_adapter,
        pair=pair,
        regime="constant",
        lag_grid=[0, 1, 2, 4],
...
def test_probe_uses_detached_branch_snapshot(test_adapter: RecurrentGemmaAdapter):
    """Verify that running 2AFC retrieval from cloned snapshots produces valid margins."""
    pair = CANONICAL_STIMULI_PAIRS[0]
    records, _ = evaluate_impulse_trajectory(
        adapter=test_adapter,
        pair=pair,
...
def test_direct_kv_residency_boundary(test_adapter: RecurrentGemmaAdapter):
    ...
    window = getattr(test_adapter.config, "attention_window_size", getattr(test_adapter.config, "sliding_window", 8))
    pair = CANONICAL_STIMULI_PAIRS[0]
    records, _ = evaluate_impulse_trajectory(
        adapter=test_adapter,
        pair=pair,
...
def test_end_to_end_all_regimes_with_layer_traces(test_adapter: RecurrentGemmaAdapter):
    """End-to-end dry run verifying that evaluation completes with summary and layer traces across all regimes."""
    pair = CANONICAL_STIMULI_PAIRS[0]

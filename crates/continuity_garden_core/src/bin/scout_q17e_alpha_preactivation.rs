//! Development Alpha Preactivation Identity Residual Scout for Q17E
//! Evaluates preactivation identity residual recurrence:
//!   z_{t+1} = tanh((W_z + alpha * I) * z_t + W_x * x_t + b_z)
//! Leaves W_x * x_t completely unscaled by any (1 - lambda) plasticity tax.
//! Sweeps alpha in [0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50] across 16 auxiliary seeds
//! under exact 120-epoch training measuring:
//! - Initial-step Jacobian sensitivity retention ||d(z_3)/d(x_1)||_F / ||d(z_2)/d(x_1)||_F
//! - Last-step Jacobian sensitivity ||d(z_3)/d(x_3)||_F (plasticity / unscaled new-edge incorporation)
//! - k=2 baseline retention
//! - k=3 directional margin pass rate & sign-flip p-value
//! - k=3 independent cloned-twin donor-state transplant assay & continuous swap effect (with separate realization)
//! - k=3 independent transposition reversal pass rate (with separate realization)
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AlphaResidualMemoryModel {
    pub w_z: Vec<f32>,
    pub w_x: Vec<f32>,
    pub b_z: Vec<f32>,
    pub w_q: Vec<f32>,
...
impl AlphaResidualMemoryModel {
    pub fn new_init(seed: u64, alpha: f32) -> Self {
    ...
    /// Step forward with preactivation identity residual:
    ///   a_{t+1} = (W_z + alpha * I) * z_t + W_x * x_t + b_z
    ///   z_{t+1} = tanh(a_{t+1})
    #[inline(always)]
    pub fn step(&self, state: &RecurrentState, obs: &TransitionObservation) -> (RecurrentState, Vec<f32>) {
        ...
        let mut dtanh = vec![0.0f32; HIDDEN_DIM];

        for i in 0..HIDDEN_DIM {
            let mut sum = self.b_z[i] + self.alpha * state.z[i];
    ...
    /// Meta-train the preactivation residual recurrent network on 2-step sequences ONLY under exact 120 epochs
    pub fn meta_train_bptt(&mut self, rng_seed: u64, epochs: usize) {
        ...
        for _ in 0..epochs {
            for _ in 0..TRAIN_BATCHES_PER_EPOCH {
                ...
                // Step 1: z_0 = 0 -> a1 = W_x * x1 + b_z -> z1 = tanh(a1)
                let mut dt1 = vec![0.0f32; HIDDEN_DIM];
                ...
                // Step 2: a2 = (W_z + alpha * I) * z1 + W_x * x2 + b_z -> z2 = tanh(a2)
                let mut dt2 = vec![0.0f32; HIDDEN_DIM];
                let mut z2 = vec![0.0f32; HIDDEN_DIM];
                for i in 0..HIDDEN_DIM {
                    let mut sum = self.b_z[i] + a * z1[i];
                    for j in 0..HIDDEN_DIM {
                ...
                // grad_z1 = (W_z + alpha * I)^T * d_a2
                let mut grad_z1 = vec![0.0f32; HIDDEN_DIM];
                for j in 0..HIDDEN_DIM {
                    let mut sum = a * d_a2[j];
                    for i in 0..HIDDEN_DIM {
...
fn evaluate_seed_alpha(seed_index: usize, alpha: f32) -> SeedAlphaRecord {
    let seed = 88000 + (seed_index as u64) * 777;
    let aux_train_seed = seed + 999;

    let mut model = AlphaResidualMemoryModel::new_init(seed, alpha);
    ...
    let (z1, dt1) = model.step(&z_k2_init, &TransitionObservation::new(a, 1, b));
    let (z2, dt2) = model.step(&z1, &TransitionObservation::new(b, 2, c));
    ...
    // k=3 Evaluation (Intact Stream Twin 1): A(1) -> B(2) -> C(3) -> D(4)
    let (z3_twin1, dt3) = model.step(&z2, &TransitionObservation::new(c, 1, d));
    ...
    // Independent Cloned-Twin Donor Stream (Separate Realization):
    // Realized on reverse path with independent alternate action sequence (action 2, action 1, action 2)
    let mut z_donor = RecurrentState::zero();
    z_donor = model.step(&z_donor, &TransitionObservation::new(d, 2, c)).0;
    z_donor = model.step(&z_donor, &TransitionObservation::new(c, 1, b)).0;
    z_donor = model.step(&z_donor, &TransitionObservation::new(b, 2, a)).0;
    ...
    // Independent Transposition Control (Separate Realization with action 1, 2, 1)
    ...
    let k3_transposition_score = model.query_composition(&z_trans, (a, d)) - model.query_composition(&z_trans, (d, a));
    ...
    // A_t = diag(dtanh_t) * (W_z + alpha * I)
    // J_1 = diag(dtanh_1) * W_x
    let mut j1 = vec![0.0f32; HIDDEN_DIM * OBS_DIM];
    for i in 0..HIDDEN_DIM {
        for j in 0..OBS_DIM {
            j1[i * OBS_DIM + j] = dt1[i] * model.w_x[i * OBS_DIM + j];
        }
    }
    ...
    for i in 0..HIDDEN_DIM {
        for l in 0..OBS_DIM {
            let mut sum = alpha * j2[i * OBS_DIM + l];
            for m in 0..HIDDEN_DIM {
                sum += model.w_z[i * HIDDEN_DIM + m] * j2[m * OBS_DIM + l];
            }
            j3[i * OBS_DIM + l] = dt3[i] * sum;
        }
    }
    let jac_init_step_k3 = j3.iter().map(|&v| v * v).sum::<f32>().sqrt();

    // Last-step Jacobian sensitivity: ||d(z_3)/d(x_3)||_F = ||diag(dt3) * W_x||_F (unscaled by 1-lambda!)
    let mut j_last = vec![0.0f32; HIDDEN_DIM * OBS_DIM];
    for i in 0..HIDDEN_DIM {
        for j in 0..OBS_DIM {
            j_last[i * OBS_DIM + j] = dt3[i] * model.w_x[i * OBS_DIM + j];
        }
    }
...
fn main() {
    println!("==================================================================================================================");
    println!("SCOUT 2: Preactivation Identity Residual Alpha Sweep across 16 Auxiliary Seeds (120 Training Epochs)");
    println!("Architecture: z_{{t+1}} = tanh((W_z + alpha * I) * z_t + W_x * x_t + b_z)  [Unscaled W_x * x_t]");
    println!("==================================================================================================================");
    println!("{:<6} | {:<8} | {:<12} | {:<8} | {:<10} | {:<8} | {:<8} | {:<10} | {:<10} | {:<8}",
        "Alpha", "k=2 Ret", "k=3 Marg (p)", "Surgery", "Mean Swap", "Transp", "Shuffle", "Init Jac Ret", "Last-Edge", "Sensor");
    println!("------------------------------------------------------------------------------------------------------------------");

    let alpha_grid = [
        0.00f32, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50
    ];

    let mut all_scout_records = Vec::new();

    for &alpha in &alpha_grid {
        let results: Vec<SeedAlphaRecord> = (1..=16)
            .into_par_iter()
            .map(|i| evaluate_seed_alpha(i, alpha))
            .collect();
    ...
    let out_file = data_dir.join("q17e_alpha_scout_results.json");
    let file = File::create(&out_file).expect("Failed to create alpha scout results JSON");
    let writer = BufWriter::new(file);
    serde_json::to_writer_pretty(writer, &all_scout_records).expect("Failed to write JSON");
    println!("Persisted full alpha sweep telemetry to: {}", out_file.display());
}

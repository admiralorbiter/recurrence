...
**Status:** **S12a.1 Exploratory Scout FROZEN; S12b Confirmatory Protocol FROZEN & PREREGISTERED**
...
### Fully Symmetrized Causal Control Battery (S12a.1 & S12b)
1. **Intact Endpoints ($S^A, S^B$):** Establish ground-truth donor/recipient contrast.
2. **Whole-Store Graft Gate ($S^{B \leftarrow A}_{\text{all}}, S^{A \leftarrow B}_{\text{all}}$):** Live check verifying that complete state transplantation reproduces donor logits to strict numerical tolerance ($D_{\text{JS}} < 10^{-4}$).
3. **Sham Control ($S^{A_1 \leftarrow A_2}, S^{B_1 \leftarrow B_2}$):** Confirms zero artifactual logit distortion ($\bar{\Delta}_{\text{sham}} = 0.00$, $P_{\text{sham}} = 0.00$, $\alpha_{\text{sham}}^{\text{logit}} = 0.000$).
4. **Intervention-Matched Frobenius Noise:** Scales random Gaussian perturbations strictly to the per-layer Frobenius norm $\|R_{\text{donor}} - R_{\text{recipient}}\|_F$ and projects resulting output displacement onto the real donor axis across multiple seeds in both directions ($B \to A$ and $A \to B$).
5. **Balanced Cyclic Derangement Cross-Pair Controls:**
   - **Primary Unrelated Control (Shift +1):** Donor pair index $(i + 1) \bmod N$ evaluated in both directions ($B \leftarrow C_A$ along $z_A - z_B$, $A \leftarrow C_B$ along $z_B - z_A$).
   - **Secondary Permuted Control (Shift +7):** Coprime cyclic shift $(i + 7) \bmod N$ evaluated symmetrically.
6. **Sliced Post-Graft KV Mediation:** Measures distances strictly over newly generated cache positions ($t > L_{\text{initial}}$) to test if continuous RG-LRU recurrence pulls sliding-window attention representations toward the donor during future unrolls, both at short horizon ($N=512$) and full window turnover ($N=2048$).
7. **Eligibility Reporting:** Explicitly displays $N_{\text{eligible}} / N_{\text{total}}$ for behavioral attribution ($|m_D - m_R| \ge 0.5$).

---

## 2. S12a.1 Exploratory Causal Scout Manifest
...
## 3. Empirical Causal Factorial Panel & Directional Logit Displacements (S12a.1 Scout)

| Lag $L$ | Intervention Condition | Signed Graft Effect $\bar{\Delta}_C$ | Directional Displacement $P_C$ | Logit Proj $\alpha_C^{\text{logit}}$ | Attrib Index $\alpha_C^{\text{cloze}}$ | Eligible Cells | Donor Concordance |
| :---: | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
...
2. **Directional Steering ($P_C$) Dynamics:**
...
- **Protocol Version:** `S12b-Confirmatory-v1.0` (Fail-closed execution).
- **Substrate:** `google/recurrentgemma-2b` (bfloat16 on CUDA).
- **Scope:** Exactly 20 canonical stimulus pairs $\times$ 4 filler regimes $\times$ 3 lag checkpoints ($L \in \{8, 2049, 4096\}$) $\times$ 22 symmetric intervention conditions = 5,280 swap records + 160 mediational unrolls ($N \in \{512, 2048\}$).
- **Primary Physical Causal Endpoint:** Directional displacement $P_{\text{RGLRU}}(2W)$ across all 20 stimulus pairs with 10,000-draw Pair-Cluster Bootstrap 95% CI.
- **Primary Specificity Estimand:** Paired contrast $\Delta P_{\text{spec\_unrel}} = P_{\text{matching}}(2W) - P_{\text{unrelated}}(2W)$ under balanced cyclic derangement ($i \to (i+1) \bmod 20$).
- **Secondary Specificity Estimand:** Paired contrast $\Delta P_{\text{spec\_perm}} = P_{\text{matching}}(2W) - P_{\text{permuted}}(2W)$ under coprime cyclic derangement ($i \to (i+7) \bmod 20$).
- **Secondary Temporal Growth Estimand:** Paired trajectory contrast $\Delta P_{\text{growth}} = P_{\text{RGLRU}}(2W) - P_{\text{RGLRU}}(W+1)$.
- **Secondary Store Contrast Estimand:** Paired contrast $\Delta P_{\text{kv\_minus\_rglru}}(2W) = P_{\text{KV}}(2W) - P_{\text{RGLRU}}(2W)$.
- **Statistical Aggregation:** Average the 4 fixed filler regimes within each pair cluster, then bootstrap the 20 pair summaries ($B=10,000$).
- **Mediation Sub-Experiment:** Sliced post-graft KV cache evaluated at $N=512$ and full attention-window turnover at $N=2048$.

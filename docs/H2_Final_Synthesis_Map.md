...
## 1. The Canonical Seven-Way Synthesis Map
...
|---|---|:---:|---|---|---|
| **1. Reconstructibility** | Can public token history reproduce hidden internal state? | **YES** | **S10 Replay Invariant:** Exact reconstruction $S_t = \mathcal{F}_\theta(x_{1:t})$ across deterministic execution runs. | Restricted to deterministic execution; does not hold under stochastic sampling. | **Hidden $\neq$ Privileged.** Internal state is private from prompt text but not informationally privileged over public history. |
...
| **8. Monitor / Content Dissociation** | Is there a second-order latent variable that monitors first-order computation without duplicating it? | **UNKNOWN** | Untested in Horizon 2 Core. | Requires $2 \times 2$ factorial intervention ($C_\pm \times M_\pm$). | **Final Mechanistic Coda (Pre-H3 Bridge).** Tests whether a second-order monitor exists before moving to development. |
| **9. Developmental Self / Source Emergence** | Can source models, self/world boundaries, and active regulation emerge through learning? | **UNKNOWN** | Untested in pretrained weights. | Requires interactive environment where persistent state is necessary for survival/prediction. | **Horizon 3 Frontier.** The fundamental shift from observational analysis of frozen weights to developmental construction. |

---

## 2. Epistemic Guardrail Summary
...
## 3. Downstream Document Integration
